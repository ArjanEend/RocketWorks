    ...
    struct ChannelPacket
    {
        ...
        public bool SendToTransport(SocketConnection conn, int channelId)
        {
            ...
            if (!conn.TransportSend(m_Buffer, (ushort)m_Position, channelId, out error))
            {
                ...
                {
                    RocketLog.Log("Failed to send internal buffer channel:" + channelId + " bytesToSend:" + m_Position);
                    result = false;
                }
            }
            if (error != 0)
            {
                ...
                RocketLog.Log("Send Error: " + error + " channel:" + channelId + " bytesToSend:" + m_Position);
                result = false;
            }
            m_Position = 0;
            return result;
